...
class unFlowLoss(nn.modules.Module):
    def __init__(self,w_l1=0.0,w_ssim=0.0,w_ternary=1.0,w_smooth=50.0,smooth_2nd=False,w_scales=[1.0, 1.0, 1.0],alpha=10,occ_from_back=False,warp_pad="border",with_bk=True,w_sm_scales=[1.0, 1.0, 1.0]):
        ...
        self.alpha=alpha
        self.warp_pad=warp_pad
        self.with_bk=with_bk
        self.w_sm_scales=w_sm_scales
    ...
    def loss_photomatric(self, im1_scaled, im1_recons, occu_mask1):
        ...
        if self.w_l1 > 0:
            loss += [self.w_l1 * (im1_scaled - im1_recons).abs() * occu_mask1]
    ...
    def loss_smooth(self, flow, im1_scaled):
        ...
        loss = []
        loss += [func_smooth(flow, im1_scaled, self.alpha)]
        return sum([l.mean() for l in loss])

    def forward(self, output, target):
        """
        output: flow list
        target: B*2 x 3 x h x w
        
        """
        bs = target.shape[0]//2
        target = torch.cat([target[:bs],target[bs:]], axis=1)
        output = [torch.cat([flow[bs:],flow[:bs]], axis=1) for flow in output]
        """
        we need:
        ...
        for i, flow in enumerate(pyramid_flows):
            ...
            im1_recons = flow_warp(im2_scaled, flow[:, :2], pad=self.warp_pad)
            im2_recons = flow_warp(im1_scaled, flow[:, 2:], pad=self.warp_pad)
        ...
        smooth_loss = self.w_smooth * sum(pyramid_smooth_losses)
