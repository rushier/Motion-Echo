...
def window_partition(x, window_size):
    ...
    windows = (
        x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size[0]*window_size[1], C).contiguous()
    )
    return windows
...
def window_reverse(windows, window_size, H, W):
    ...
    windows = windows.view(-1, window_size[0], window_size[1], C).contiguous()
...
class InterFrameAttention(nn.Module):
    ...
    def forward(self, x1, x2, cor, H, W, mask=None):
        B, N, C = x1.shape
        B, N, C_c = cor.shape
        q = self.q(x1).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
        kv = self.kv(x2).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()
        cor_embed_ = self.cor_embed(cor)
        cor_embed = cor_embed_.reshape(B, N, self.num_heads, self.motion_dim // self.num_heads).permute(0, 2, 1, 3).contiguous()
        k, v = kv[0], kv[1]
        attn = (q @ k.transpose(-2, -1).contiguous()) * self.scale
        #print(x1.shape, q.shape, k.transpose(-2, -1).shape, attn.shape)

        if mask is not None:
            nW = mask.shape[0] # mask: nW, N, N
            attn = attn.view(B // nW, nW, self.num_heads, N, N).contiguous() + mask.unsqueeze(
                1
            ).unsqueeze(0)
        ...
        c_reverse = (attn @ cor_embed).transpose(1, 2).reshape(B, N, -1).contiguous()
...
class MotionFormerBlock(nn.Module):
    def __init__(self, dim, motion_dim, num_heads, window_size=0, shift_size=0, mlp_ratio=4., bidirectional=False, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
    ...
    def forward(self, x, cor, H, W, B):
        ...
        #x_norm, x_reverse = x_norm_full[:nwB//2], x_norm_full[nwB//2:]
        ...
        x_depad = depad_if_needed(x_back_win, x.size(), self.window_size).view(2*B, H * W, -1).contiguous()
        x_motion = depad_if_needed(x_motion, cor.size(), self.window_size).view(2*B, H * W, -1).contiguous()
            
        x_depad = x_depad + self.drop_path(self.mlp(self.norm2(x_depad), H, W))
        return x_depad, x_motion
...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        #print('overleap pathch embed', x.shape)
        return x, H, W
...
class MotionFormer(nn.Module):
    def __init__(self, in_chans=3, embed_dims=[16, 32, 64, 128, 256], motion_dims=[0, 0, 0, 32, 64], num_heads=[4, 8], 
                 mlp_ratios=[4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[2, 2, 2, 2, 2], window_sizes=[7, 7],**kwarg):
        # in_chans=3, embed_dims=[16, 32, 64, 128, 256], motion_dims=[0, 0, 0, 32, 64], num_heads=[8, 16], 
                 # mlp_ratios=[4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 # attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 # depths=[2, 2, 2, 4, 4], window_sizes=[7, 7],**kwarg
        super().__init__()
        self.depths = depths
    ...
    def get_cor(self, shape, device):
        ...
        if k not in self.cor:
            tenHorizontal = torch.linspace(-1.0, 1.0, shape[2], device=device).view(
                1, 1, 1, shape[2]).expand(shape[0], -1, shape[1], -1).permute(0, 2, 3, 1).contiguous()
            tenVertical = torch.linspace(-1.0, 1.0, shape[1], device=device).view(
                1, 1, shape[1], 1).expand(shape[0], -1, -1, shape[2]).permute(0, 2, 3, 1).contiguous()
...
class my_attention(nn.Module):
    # def __init__(self, embed_dims=[32, 64, 128, 256], motion_dims=[32, 32, 32, 32], num_heads=[2, 2, 4, 4], 
    #              mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
    #              attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
    #              depths=[2, 2, 2, 2], window_sizes=[7, 7, 7, 7]):
    def __init__(self, embed_dims=[32, 64, 128], motion_dims=[32, 32, 32], num_heads=[2, 2, 4], 
                 mlp_ratios=[4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[2, 2, 2], window_sizes=[7, 7, 7]):
        super().__init__()
        ...
        self.block0 = nn.ModuleList([MotionFormerBlock(
                        dim=embed_dims[0], motion_dim=motion_dims[0], num_heads=num_heads[0], window_size=window_sizes[0], 
                        shift_size= 0 if (j % 2) == 0 else window_sizes[0] // 2,
                        mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer) for j in range(depths[0])])
        self.norm0 = norm_layer(embed_dims[0])
                                     
        self.block1 = nn.ModuleList([MotionFormerBlock(
                        dim=embed_dims[1], motion_dim=motion_dims[1], num_heads=num_heads[1], window_size=window_sizes[1], 
                        shift_size= 0 if (j % 2) == 0 else window_sizes[1] // 2,
                        mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer) for j in range(depths[1])])
        self.norm1 = norm_layer(embed_dims[1])
                                     
        self.block2 = nn.ModuleList([MotionFormerBlock(
                        dim=embed_dims[2], motion_dim=motion_dims[2], num_heads=num_heads[2], window_size=window_sizes[2], 
                        shift_size= 0 if (j % 2) == 0 else window_sizes[2] // 2,
                        mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer) for j in range(depths[2])])
        self.norm2 = norm_layer(embed_dims[2])
                                     
        # self.block3 = nn.ModuleList([MotionFormerBlock(
        #                 dim=embed_dims[3], motion_dim=motion_dims[3], num_heads=num_heads[3], window_size=window_sizes[3], 
        #                 shift_size= 0 if (j % 2) == 0 else window_sizes[3] // 2,
        #                 mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
        #                 drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer) for j in range(depths[3])])
    ...
    def forward_(self, x, block, norm):
        motion_fea = []
        B0, _, H, W = x.shape
        B = B0//2
        cor = self.get_cor((x.shape[0], H, W), x.device)
        for blk in block:
            x, x_motion = blk(x, cor, H, W, B)
            motion_fea.append(x_motion.reshape(2*B, H, W, -1).permute(0, 3, 1, 2).contiguous())
        x = norm(x)
        x = x.reshape(2*B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        motion = torch.cat(motion_fea, 1)
        return x, motion
    
    def forward(self, xs):
        
        af0, mf0 = self.forward_(xs[0], self.block0, self.norm0)
        af1, mf1 = self.forward_(xs[1], self.block1, self.norm1)
        af2, mf2 = self.forward_(xs[2], self.block2, self.norm2)
        # af3, mf3 = self.forward_(xs[3], self.block3, self.norm3)
        appearance_features = [af0, af1, af2]
        motion_features = [mf0, mf1, mf2]
        
        return appearance_features, motion_features
    
class my_attention_add(nn.Module):
    def __init__(self, embed_dims=[32, 64, 128, 256], motion_dims=[32, 32, 32, 32], num_heads=[2, 2, 4, 4], 
                 mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[2, 2, 2, 2], window_sizes=[7, 7, 7, 7]):
    ...
    def forward(self, xs):
        ...
        appearance_features = [af0 + xs[0], af1 + xs[1], af2 + xs[2], af3 + xs[3]]
        motion_features = [mf0, mf1, mf2, mf3]
        
        return appearance_features, motion_features
    
    
class my_attention_TransUnet_add(nn.Module):
    ...
    def forward(self, xs):
        ...
        af1, mf1 = self.forward_(xs[1], self.block1, self.norm1)
        af2, mf2 = self.forward_(xs[2], self.block2, self.norm2)
        appearance_features = [af0 + xs[0], af1 + xs[1], af2 + xs[2]]
        motion_features = [mf0, mf1, mf2]
        # appearance_features = [af0 + xs[0], af1 + xs[1]]
        # motion_features = [mf0, mf1]
        
        return appearance_features, motion_features
    
    
class my_attention_TransUnet(nn.Module):
    def __init__(self, embed_dims=[512, 256, 64], motion_dims=[32, 32, 32], num_heads=[2, 4, 4], 
                 mlp_ratios=[4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[2, 2, 2], window_sizes=[7, 7, 7]):
