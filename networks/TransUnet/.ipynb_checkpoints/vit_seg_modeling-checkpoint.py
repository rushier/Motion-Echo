...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        return  logits, logits, logits, logits, chamber_pred
...
class VisionTransformer_cst_temporal(nn.Module):
    def __init__(self, args, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(VisionTransformer_cst_temporal, self).__init__()
        self.args = args
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
        ...
        if self.args.classify:
            ...
            self.classify_head = nn.Linear(512, 4)
        if args.SELayer:
            self.summary = channel_attn.SELayer(256)
    ...
    def forward(self, x):
        ...
        #     fea = features[i].clone()
        #     fea = fea.view(-1, args.temporal_interval*2+1, fea.size(1),fea.size(2),fea.size(3))
        #     fea_out = self.summary(fea)
        #     fea_out = fea_out.expand_as(fea)
        #     features[i] = torch.stack([fea, fea_out], axis=1)
...
class VisionTransformer_cst_temporal_flow(nn.Module):
    ...
    def forward(self, imgs):
        ...
        x = self.decoder(x, appearance_attn)
        logits = self.segmentation_head(x)
        warped_imgs, warped_logits, flow_list = self.flow_estimation(imgs, logits, appearance_attn, motion_attn)
        q = self.proj_head(x)
        if self.args.classify:
            x_cls = self.avg_pool(features[0]).view(features[0].size(0), -1)
            chamber_pred = self.classify_head(x_cls)
            ...
            chamber_pred = torch.randn(x.shape[0],4).to(logits.device)
        return flow_list, logits, warped_imgs, warped_logits, chamber_pred
...
class VisionTransformer_cst_temporal_flow_woResidual(nn.Module):
    ...
    def forward(self, imgs):
        ...
        logits = self.segmentation_head(x)
        warped_imgs, warped_logits, flow_list = self.flow_estimation(imgs, logits, appearance_attn, motion_attn)
        # warped_imgs and warped_logits are reversed
        q = self.proj_head(x)
        if self.args.classify:
            x_cls = self.avg_pool(features[0]).view(features[0].size(0), -1)
            chamber_pred = self.classify_head(x_cls)
    ...
    def forward_4flowPrediction(self, imgs, masks):
        if imgs.size()[1] == 1:
            imgs = imgs.repeat(1,3,1,1)
        x, attn_weights, features = self.transformer(imgs)  # (B, n_patch, hidden)
        appearance_attn, motion_attn = self.attn(features[:3])
        appearance_attn.extend(features[3:])
        warped_imgs, warped_logits, flow_list = self.flow_estimation(imgs, masks, appearance_attn, motion_attn)
        # warped_imgs and warped_logits are reversed
        return flow_list, warped_imgs, warped_logits
